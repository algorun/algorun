...
module.exports = {
  ...
  runv2_0: function (input_data, manifest, callback) {
    try {
        ...
        caching = manifest['caching'] == 'on';

        if(manifest['algo_output'] && manifest['algo_output'].length > 0){
            output_files = manifest['algo_output'];
            output_dict = {}
            error = false;
            if(!caching || cache_table.indexOf(algo_hash) == -1){
                var result = execSync('ls')
                if(result.stderr){
                    error = true
                }
                else{
                    cache_table = cache_table.concat(algo_hash)
                }
            }
            if(!error){
            ...
            //             	} else {
	           ...
	           //                  output_dict[output.name] = result;
	           //              }
            //                 fs.unlinkSync(process.env.CODE_HOME + '/src/' + output.src)
            //                 }
            //         }
            //         callback('text', output_dict);
            //     } else {
