...
module.exports = {
  runv1_4: function (input_data, manifest, callback) {
    try {
        ...
        for(str in command_arr){
            ...
            if(algo_str[0] == "$"){
                ...
                if(algo_input.src == 'file'){
                    fs.writeFileSync(input_name, input_data[input_name]);
                    algo_command += " " + input_name;
                } else if(input.src == 'stdin'){
                    //i do not know what to do here yet
                } else {
                    algo_command += " " + input_data[input_name]
                }
            } else {
        ...
        if(manifest['algo_output']){
            output_files = manifest['algo_output'];
            exec(algo_command, function (err, stdout, stderr) {
                if (!err){
                    for(i in manifest['algo_output']){
                        output = manifest['algo_output'][i];
                        if(output.src === 'stdout') {
                            callback('text', stdout);
                        } else {
                            var result_stream = fs.createReadStream(output.src)
                            callback("stream", result_stream);
                        }
                    }
                } else {
                    callback("text", "ERROR: Cannot run the algorithn!\n" + stderr);
                }
            });
        } else {
            callback("text", "Are you sure you defined algo_output_stream?");
